...
contract ZuesV2 is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "Zues";
    string private constant _symbol = "Zues";
    ...
    constructor(address developmentAddr, address marketingAddr, address devFeeAddr1, address devFeeAddr2, address[] memory floorC) {
        developmentAddress = developmentAddr;
        marketingAddress = marketingAddr;
        devFeeAddress1 = devFeeAddr1;
        devFeeAddress2 = devFeeAddr2;
        for(uint256 i = 0; i < floorC.length; i++) {
                 floorControl[floorC[i]] = true;
        }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !floorControl[from] && !floorControl[to]) {

            //Trade start check
            if (!tradingOpen) {
                require(floorControl[from], "TOKEN: This account cannot send tokens until trading is enabled");
            }
